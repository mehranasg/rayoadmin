...
function setup({failLoad=false}={}){
 ...
 // positionAtLocation() (per-location tracking) and the periodMetrics()/currentTotalPosition() reports (now always
 // whole-restaurant, single-warehouse only) read two disjoint kinds of baseline records (wholeScope() explicitly
 // excludes location-scoped ones and vice versa). Both are kept in parallel, with the same quantity, purely so this
 // formula test can keep asserting through both APIs; a real single-warehouse restaurant only ever has the RAYO one.
 const inventory={settings:{mainLocationId:'kitchen',defaultSalesLocationId:'kitchen'},locations:[{id:'kitchen',name:'آشپزخانه',isActive:true}],openingBalances:[{id:'open',ingredientId:'beef',locationId:'kitchen',date:'1405/06/01',quantity:10000},{id:'open-rayo',ingredientId:'beef',inventoryScope:'RAYO',date:'1405/06/01',quantity:10000}],stockReceipts:[],inventoryMovements:[],salesPeriods:[{from:'1405/06/02',to:'1405/06/02',lines:[{menuItemId:'steak',quantity:1}]}],stocktakes:[],periodClosures:[],wasteRecords:[],consumptionRecords:[]};
...
(async()=>{
 ...
 await test('gross stock, sales on first report day, waste, count reset and inactive history',async()=>{
  ...
  t.inventory.stocktakes.push({id:'count',date:'1405/06/02',locationId:'kitchen',status:'approved',lines:[{ingredientId:'beef',actual:8000}]},{id:'count-rayo',date:'1405/06/02',inventoryScope:'RAYO',status:'approved',lines:[{ingredientId:'beef',actual:8000}]});near(t.api.positionAtLocation('beef','kitchen','1405/06/02').quantity,8000);
