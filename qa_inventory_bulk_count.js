...
(async()=>{
 ...
 t.inputs[0].value='3';await t.api.saveBulkCount();assert.equal(t.saves(),2,'same-day overlap is replaced directly');assert.equal(t.state.stocktakes.length,1);assert.equal(t.state.stocktakes[0].lines.find(x=>x.ingredientId==='a').actual,3);assert.equal(t.state.stocktakes[0].revisionHistory.length,1,'prior count is versioned');assert.equal(t.state.stocktakes[0].status,'approved','replacement is approved immediately');
 ...
 t=setup();t.state.settings.countApprovalRequired=true;t.els.v1012BulkOpening.checked=true;t.inputs[0].value='2';await t.api.saveBulkCount();assert.equal(t.state.stocktakes[0].status,'approved','bulk count bypasses the legacy approval setting');assert.equal(t.state.stocktakes[0].lines[0].variance,0);
 ...
 console.log('PASS bulk count: collapsed direct entry, zero, decimals, direct approval, same-day versioned edit without prompt, later-date guard, retry identity, optional opening note and closed period');
