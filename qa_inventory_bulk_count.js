...
function setup(){
 ...
 const els={v1012BulkDate:{value:'۱۴۰۵/۰۶/۲۴'},v1012BulkLoc:{value:'main'},v1012BulkOpening:{checked:false},v1012BulkNote:{value:'شمارش آزمایشی'},v1012BulkFields:{disabled:false},v1012BulkStatus:{textContent:''}};
 let saves=0,failure=null,backups=0,confirmations=0,confirmResult=true;const errors=[];
 const position=()=>({quantity:10,unitCost:2,baseDate:'1405/06/01',baseQuantity:10,movementNet:0,salesConsumption:0});
 const context={console,URLSearchParams,location:{pathname:'/test.html',search:''},sessionStorage:{getItem:()=> 'admin'},confirm:()=>{confirmations++;return confirmResult},document:{readyState:'loading',querySelector:()=>null,createElement:()=>({}),head:{appendChild(){}},addEventListener(){},getElementById:id=>els[id],querySelectorAll:s=>s==='[data-v1012-bulk-qty]'?inputs:[]},setTimeout(){},getInventoryState:()=>state,getPricingState:()=>({ingredients:items}),RayoJalali:{today:()=> '1405/06/24'},RayoInventoryV10:{isReady:()=>true,positionAtLocation:position,positionBeforeDate:position,downloadInventoryJson:()=>backups++},RAYO_API_GATEWAY:{createRequestId:()=>String(saves+1),saveModule:async(module)=>{assert.equal(module,'inventory');saves++;await Promise.resolve();if(failure)throw failure},loadModule:async()=>{throw Error('offline')}},toast:(msg,error)=>{if(error)errors.push(msg)}};
 ...
 return {state,items,inputs,prices,els,context,errors,api:context.RayoInventoryV1012,saves:()=>saves,backups:()=>backups,confirmations:()=>confirmations,confirmResult:v=>confirmResult=v,fail:e=>failure=e};
}
...
(async()=>{
 searchChecks();
 assert(code.includes('<details class="card v1012-direct-entry"><summary><b>ورود مستقیم به انبار</b></summary>'),'direct-entry card starts collapsed and opens from its title');
 ...
 t.inputs[0].value='3';await t.api.saveBulkCount();assert.equal(t.saves(),2,'same-day overlap is replaced');assert.equal(t.state.stocktakes.length,1);assert.equal(t.state.stocktakes[0].lines.find(x=>x.ingredientId==='a').actual,3);assert.equal(t.state.stocktakes[0].revisionHistory.length,1,'prior count is versioned');assert.equal(t.backups(),1,'replacement downloads a backup');assert.equal(t.confirmations(),1,'replacement has a preview confirmation');
 t.els.v1012BulkDate.value='1405/06/23';t.inputs[0].value='4';await t.api.saveBulkCount();assert.equal(t.saves(),2,'count before a later count remains blocked');
 t=setup();t.inputs[0].value='2';await t.api.saveBulkCount();t.inputs[0].value='8';t.confirmResult(false);await t.api.saveBulkCount();assert.equal(t.saves(),1,'cancelled replacement does not save');assert.equal(t.state.stocktakes[0].lines[0].actual,2);assert.equal(t.backups(),0,'cancelled replacement does not download a backup');
 ...
 t=setup();t.els.v1012BulkOpening.checked=true;t.els.v1012BulkNote.value='';t.inputs[0].value='2';await t.api.saveBulkCount();assert.equal(t.saves(),1,'opening description is optional');assert.equal(t.state.stocktakes[0].notes,'');
 ...
 console.log('PASS bulk count: collapsed direct entry, zero, decimals, same-day versioned replacement, preview, backup, later-date guard, retry identity, optional opening note and closed period');
