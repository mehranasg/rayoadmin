...
function hrDataRequiredForPage(){const f=(location.pathname.split('/').pop()||'index.html').toLowerCase();return f===''||f==='index.html'||f==='personnel.html'||f==='reports.html'}
async function init(){showLoading(true);try{if(hrDataRequiredForPage()){const live=await StorageAdapter.load(),st=window.RAYO_API_GATEWAY?.getModuleStatus?.('hr')||{};state=migrate(live);window.__RAYO_HR_SERVER_CONFIRMED__=st.initialized===true;$('saveStatus').textContent=st.initialized===true?'اطلاعات زنده از RayoData دریافت شد':'ماژول پرسنل هنوز مقداردهی اولیه نشده است — ذخیره عادی مسدود است'}else{state=migrate(structuredClone(DEFAULT_DATA));window.__RAYO_HR_SERVER_CONFIRMED__=false;$('saveStatus').textContent=''}const t=parseJDate(todayJ());ui.payroll.year=ui.payslip.year=ui.reports.year=ui.monthlyPlan.year=ui.annualCalendar.year=t.y;ui.payroll.month=ui.payslip.month=ui.reports.month=ui.monthlyPlan.month=t.m;ui.shiftWeekStart=todayJ();ui.monthlyPlan.selectedDate=todayJ();ui.staffingMap.date=todayJ();ui.payments.year=t.y;ui.payments.month=t.m;render()}catch(e){window.__RAYO_HR_SERVER_CONFIRMED__=false;toast(e.message,true);state=migrate(structuredClone(DEFAULT_DATA));render();$('saveStatus').textContent='خطا در دریافت اطلاعات سرور — ذخیره برای حفاظت از داده‌ها غیرفعال است'}finally{showLoading(false)}}
...
function tipEligibleV5(type){const active=activePersonnel();if(type==='کلی')return [...active].sort((a,b)=>a.name.localeCompare(b.name,'fa'));if(type==='سالن')return active.filter(p=>p.personnelGroup==='سالن'&&['سالن بالا','سالن پایین','بار','هاست'].includes(p.section)).sort((a,b)=>a.section.localeCompare(b.section,'fa')||a.name.localeCompare(b.name,'fa'));return active.filter(p=>p.personnelGroup!=='آشپزخانه').sort((a,b)=>{const rank=p=>p.mainPosition==='پیک'||p.section==='پیک'?0:p.mainPosition==='پارک‌بان'||p.section==='پارکینگ'?1:p.personnelGroup==='سالن'?2:3;return rank(a)-rank(b)||a.name.localeCompare(b.name,'fa')})}
...
function refreshTipPeopleV5(first=false){const type=$('tip_type').value,people=tipEligibleV5(type),existing=window.tipEditV5?.existing||new Map();$('tipHelpV5').textContent=type==='فردی'?'آشپزخانه نمایش داده نمی‌شود؛ پیک و پارک‌بان در ابتدای فهرست هستند.':type==='سالن'?'پرسنل فعال سالن بالا، سالن پایین، هاست و بار به‌صورت پیش‌فرض انتخاب می‌شوند.':'تمام پرسنل فعال به‌صورت پیش‌فرض انتخاب می‌شوند.';$('tipPeople').innerHTML=people.map(p=>{const auto=type!=='فردی',checked=existing.has(p.id)||(auto&&!first?true:auto&&first&&!window.tipEditV5?.id);const priority=p.mainPosition==='پیک'||p.section==='پیک'?'پیک':p.mainPosition==='پارک‌بان'||p.section==='پارکینگ'?'پارک‌بان':'';return `<label class="multi-row"><input type="checkbox" value="${p.id}" ${checked?'checked':''}><span><b>${esc(p.name)}</b> ${priority?`<span class="tip-priority">${priority}</span>`:''}<br><small>${p.id} — ${esc(p.personnelGroup)} / ${esc(p.section)} / ${esc(p.mainPosition)}</small></span><input type="number" step="0.1" min="0.1" value="${existing.get(p.id)||1}"></label>`}).join('')}
