...
let initialViewReady=document.readyState==='complete';
if(!initialViewReady){
  document.documentElement.classList.add('rayo-starting');
  const finishInitialView=()=>setTimeout(()=>{
    initialViewReady=true;
    try{renderView()}finally{document.documentElement.classList.remove('rayo-starting');showLoading(!state)}
  },0);
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',finishInitialView,{once:true});
  else finishInitialView();
}
...
function hrDataRequiredForPage(){const f=(location.pathname.split('/').pop()||'index.html').toLowerCase();return !tipsOnlyPage()&&(f===''||f==='index.html'||f==='personnel.html'||f==='settings.html'||f==='reports.html')}
...
function showLoading(v){$('loading').classList.toggle('show',v||!initialViewReady)}function toast(msg,error=false){msg=String(msg??'');const key=(error?'E:':'I:')+msg,now=Date.now(),last=__toastRecent.get(key)||0;if(now-last<5000)return;__toastRecent.set(key,now);const d=document.createElement('div');d.className='toast'+(error?' error':'');d.textContent=msg;$('toasts').appendChild(d);setTimeout(()=>d.remove(),error?7000:3500)}
...
function renderView(){if(!initialViewReady)return;if(!state||!views[currentView]){if($('topTitle'))$('topTitle').textContent=titles[currentView]||'در حال بارگذاری';if($('view'))$('view').innerHTML='<div class="card"><div class="empty">در حال بارگذاری اطلاعات…</div></div>';return}if(!Array.isArray(state.personnel))state.personnel=[];if(!Array.isArray(state.payrollClosures))state.payrollClosures=[];$('topTitle').textContent=titles[currentView]||'';const fn=views[currentView]||views.dashboard;$('view').innerHTML=fn();setTimeout(afterRender,0)}
...
function viewsDashboard(){const t=parseJDate(todayJ()),rows=state.personnel.filter(p=>p.status==='فعال').map(p=>calcPayroll(p,t.y,t.m));const tips=state.tipGroups.filter(x=>{const q=periodOf(x.receiveDate);return q.year===t.y&&q.month===t.m&&x.status!=='ابطال‌شده'});const totalNet=rows.reduce((s,x)=>s+x.netToman,0),out=rows.reduce((s,x)=>s+x.unpaid,0),tipTotal=tips.reduce((s,x)=>s+n(x.totalAmount),0),incompleteCount=rows.filter(x=>x.hoursIncomplete).length;return pageHead('داشبورد مدیریتی',`وضعیت ${monthName(t.m)} ${t.y}`)+`<div class="grid grid-4"><div class="card kpi"><div class="label">پرسنل فعال</div><div class="value">${activePersonnel().length}</div><div class="sub">نفر</div></div><div class="card kpi"><div class="label">حقوق خالص پیش‌بینی</div><div class="value">${money(totalNet)}</div><div class="sub">تومان${incompleteCount?` — ${incompleteCount} نفر نیازمند ساعات تقریبی`:''}</div></div><div class="card kpi"><div class="label">مانده قابل پرداخت</div><div class="value">${money(out)}</div><div class="sub">تومان</div></div><div class="card kpi"><div class="label">انعام دریافت‌شده</div><div class="value">${money(tipTotal)}</div><div class="sub">تومان</div></div></div><div class="grid grid-2"><div class="card"><div class="section-head"><h2>روند ۱۲ ماهه</h2></div><canvas class="chart" id="dashboardChart"></canvas></div><div class="card"><div class="section-head"><h2>آخرین تغییرات</h2></div>${state.changeLog.length?`<table class="mini-table">${state.changeLog.slice(0,8).map(x=>`<tr><td>${esc(new Date(x.at).toLocaleString('fa-IR'))}</td><td>${esc(x.action)}</td></tr>`).join('')}</table>`:'<div class="empty">هنوز تغییری ثبت نشده است.</div>'}</div></div>`}
...
function adjFor(pid,y,m){let a=state.monthlyAdjustments.find(x=>x.personnelId===pid&&x.year===y&&x.month===m);if(!a){a={id:uid('ADJ',state.monthlyAdjustments),personnelId:pid,year:y,month:m,hoursOverride:'',estimatedHours:'',overtimePerformance:0,otherPayment:0,otherDeduction:0,notes:''};state.monthlyAdjustments.push(a)}return a}
...
function calcPayrollLive(p,y,m,memo={}){const key=`${p.id}|${y}|${m}`;if(memo[key])return memo[key];const a=state.monthlyAdjustments.find(x=>x.personnelId===p.id&&x.year===y&&x.month===m)||{hoursOverride:'',estimatedHours:'',overtimePerformance:0,otherPayment:0,otherDeduction:0,notes:''},prev=prevPeriod(y,m),prevRow=(a.openingBalanceToman!==undefined&&a.openingBalanceToman!==null&&a.openingBalanceToman!==''?{unpaid:n(a.openingBalanceToman)}:y>1300&&periodHasData(p.id,prev.year,prev.month)?calcPayroll(p,prev.year,prev.month,memo):{unpaid:0});const penalties=state.penaltiesRewards.filter(x=>x.personnelId===p.id&&x.year===y&&x.month===m&&x.status==='تاییدشده');const penalty=penalties.filter(x=>x.type==='جریمه').reduce((s,x)=>s+n(x.amount),0),reward=penalties.filter(x=>x.type==='تشویقی').reduce((s,x)=>s+n(x.amount),0);const payments=state.payments.filter(x=>x.status!=='void'&&!x.tipSettlement&&x.personnelId===p.id&&Number(x.salaryYear)===Number(y)&&Number(x.salaryMonth)===Number(m)),advance=payments.filter(x=>x.type==='مساعده').reduce((s,x)=>s+n(x.amount),0),paid=payments.filter(x=>x.type!=='مساعده').reduce((s,x)=>s+n(x.amount),0);const cons=state.consumptions.filter(x=>x.personnelId===p.id&&x.year===y&&x.month===m),creditConsumed=cons.filter(x=>x.type==='اعتبار').reduce((s,x)=>s+n(x.amount),0),mealConsumed=cons.filter(x=>x.type==='غذای پرسنلی').reduce((s,x)=>s+n(x.amount),0),creditLimit=n(p.monthlyCredit)||n(state.settings.defaultMonthlyCredit),mealLimit=n(p.mealCredit)||n(state.settings.defaultMealCredit),creditRemaining=creditLimit-creditConsumed,mealRemaining=mealLimit-mealConsumed;const shiftRows=state.shiftRecords.filter(x=>x.personnelId===p.id&&periodOf(x.date).year===y&&periodOf(x.date).month===m&&!['لغوشده','مرخصی','غیبت'].includes(x.status)),derivedHours=shiftRows.reduce((s,x)=>s+n(x.actualHours!==''?x.actualHours:x.scheduledHours),0),estimatedHoursSet=a.estimatedHours!==''&&a.estimatedHours!==null&&a.estimatedHours!==undefined,usedEstimatedHours=a.hoursOverride===''&&shiftRows.length===0&&estimatedHoursSet,hours=a.hoursOverride!==''?n(a.hoursOverride):(shiftRows.length>0?derivedHours:(estimatedHoursSet?n(a.estimatedHours):0));const delayHours=state.delays.filter(x=>x.personnelId===p.id&&x.year===y&&x.month===m).reduce((s,x)=>s+calcDelay(x).hours,0),hourly=n(p.hourlyRate)||n(state.settings.baseHourlyRate),fixedContract=p.salaryContract?.type==='fixedMonthly'&&n(p.salaryContract.monthlySalary)>0,hoursIncomplete=!fixedContract&&a.hoursOverride===''&&shiftRows.length===0&&!estimatedHoursSet,wage=fixedContract?n(p.salaryContract.monthlySalary):Math.max(0,hours-delayHours)*hourly;const tips=state.tipGroups.filter(g=>{const q=periodOf(g.receiveDate);return q.year===y&&q.month===m&&g.status!=='ابطال‌شده'}).reduce((s,g)=>s+(tipShares(g).find(x=>x.personnelId===p.id)?.amount||0),0);const transport=n(p.transportMonthly)||n(state.settings.defaultTransport),fixed=n(p.fixedAllowance),overtime=n(a.overtimePerformance),otherPay=n(a.otherPayment),otherDed=n(a.otherDeduction),gross=wage+overtime+transport+fixed+reward+tips+otherPay,insurance=p.insurance==='دارد'?(wage+overtime+fixed)*n(state.settings.employeeInsuranceRate):0,netToman=Math.max(0,gross-penalty-insurance-otherDed-Math.max(0,-creditRemaining)-Math.max(0,-mealRemaining)),unpaid=n(prevRow.unpaid)+netToman-advance-paid,status=unpaid<=.5?'تسویه‌شده':(advance+paid>0?'پرداخت ناقص':'پرداخت‌نشده');const row={personnelId:p.id,year:y,month:m,previousBalance:n(prevRow.unpaid),penalty,reward,advance,paid,creditConsumed,creditRemaining,mealRemaining,transport,hours,usedEstimatedHours:usedEstimatedHours&&!fixedContract,hoursIncomplete,delayHours,hourly,overtime,fixed,otherPayment:otherPay,otherDeduction:otherDed,wage,tips,gross,insurance,netRial:netToman*n(state.settings.tomanToRial),netToman,unpaid,status,notes:a.notes};memo[key]=row;return row}

function calcPayroll(p,y,m,memo={}){const c=payrollClosure(y,m),r=c?.rows?.find(x=>x.personnelId===p.id);return r?payrollSettlementRow(r,c):calcPayrollLive(p,y,m,memo)}
function payrollPeopleForPeriod(y,m){return state.personnel.filter(p=>p.status==='فعال'||periodHasData(p.id,y,m))}
function payrollRowsForPeriod(y,m){const c=payrollClosure(y,m);return c?(c.rows||[]).map(r=>payrollSettlementRow(r,c)):payrollPeopleForPeriod(y,m).map(p=>calcPayrollLive(p,y,m,{}))}
...
function viewsPayroll(){const y=ui.payroll.year,m=ui.payroll.month,c=payrollClosure(y,m),rows=payrollRowsForPeriod(y,m),actions=c?`<button class="btn" onclick="showPayrollClosure()">مشاهده سند قطعی</button> <button class="btn btn-danger" onclick="reopenPayrollMonth()">بازگشایی ماه</button>`:`<button class="btn btn-success" onclick="finalizePayrollMonth()">✓ قطعی‌کردن حقوق ماه</button>`;return pageHead('کارکرد ماه و محاسبه حقوق',c?'این ماه قطعی است و اعداد از Snapshot ذخیره‌شده خوانده می‌شوند.':'مقادیر رویدادی از انعام، جریمه، پرداخت، مصرف و تأخیر خودکار جمع می‌شوند.',actions)+`<div class="card"><div class="toolbar"><div class="field"><label>سال</label><input type="number" value="${y}" onchange="ui.payroll.year=+this.value;renderView()"></div><div class="field"><label>ماه</label><select onchange="ui.payroll.month=+this.value;renderView()">${state.lists.months.map((x,i)=>`<option value="${i+1}" ${i+1===m?'selected':''}>${x}</option>`).join('')}</select></div></div>${c?`<div class="closed-banner"><span class="lock-icon">🔒</span> نهایی‌شده توسط <b>${esc(c.finalizedBy||'—')}</b> در ${esc(new Date(c.finalizedAt).toLocaleString('fa-IR'))}. پرداخت‌های همان زمان نیز داخل سند ذخیره شده‌اند.</div>`:'<div class="hint">بعد از کنترل ساعت‌ها، کسورات و پرداخت‌ها، ماه را قطعی کنید تا تغییر تنظیمات آینده روی آن اثر نگذارد.</div>'}${(()=>{const incomplete=rows.filter(x=>x.hoursIncomplete).length;return incomplete?`<div class="hint">⚠ ${incomplete} نفر ساعتی نه سابقه شیفت دارند نه ساعات تقریبی ماه ثبت‌شده؛ سهم ساعتی حقوق آن‌ها فعلاً صفر محاسبه شده است، نه پیش‌بینی کامل.</div>`:''})()}</div><div class="card"><div class="table-wrap"><table class="data-table"><thead><tr><th>پرسنل</th><th>مانده قبل</th><th>جریمه</th><th>تشویقی</th><th>مساعده</th><th>پرداختی</th><th>ساعت</th><th>تأخیر</th><th>نرخ ساعتی</th><th>حقوق ساعت</th><th>انعام</th><th>ناخالص</th><th>بیمه</th><th>خالص (ریال)</th><th>مانده پرداخت</th><th>وضعیت</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.personnelCode&&r.personnelName?`${r.personnelCode} | ${r.personnelName}`:personLabel(r.personnelId))}</td><td class="num">${money(r.previousBalance)}</td><td class="num">${money(r.penalty)}</td><td class="num">${money(r.reward)}</td><td class="num">${money(r.advance)}</td><td class="num">${money(r.paid)}</td><td>${r.hoursIncomplete?'<span class="badge warn">نیازمند تعیین ساعات تقریبی</span>':`${dec(r.hours)}${r.usedEstimatedHours?' <small class="muted">(تخمینی)</small>':''}`}</td><td>${dec(r.delayHours)}</td><td class="num">${money(r.hourly)}</td><td class="num">${money(r.wage)}</td><td class="num">${money(r.tips)}</td><td class="num">${money(r.gross)}</td><td class="num">${money(r.insurance)}</td><td class="num">${money(r.netRial)}</td><td class="num">${money(r.unpaid)}</td><td>${badge(r.status)}</td><td>${c?'<span class="badge success">قفل</span>':`<button class="btn btn-sm" onclick="editMonthlyAdjustment('${r.personnelId}')">ورودی دستی</button>`}</td></tr>`).join('')||'<tr><td colspan="17" class="empty">رکوردی برای این ماه وجود ندارد.</td></tr>'}</tbody></table></div></div>${(()=>{const versions=state.payrollClosures.filter(x=>Number(x.year)===Number(y)&&Number(x.month)===Number(m)).sort((a,b)=>String(b.finalizedAt||'').localeCompare(String(a.finalizedAt||'')));return versions.length?`<div class="card"><div class="section-head"><h2>نسخه‌های ذخیره‌شده این ماه</h2></div><div class="table-wrap"><table class="data-table"><thead><tr><th>نسخه</th><th>زمان نهایی‌سازی</th><th>نهایی‌کننده</th><th>وضعیت</th><th>مانده</th><th></th></tr></thead><tbody>${versions.map(v=>`<tr><td>${v.version}</td><td>${esc(new Date(v.finalizedAt).toLocaleString('fa-IR'))}</td><td>${esc(v.finalizedBy||'—')}</td><td>${v.active!==false?'<span class="badge success">فعال/قطعی</span>':'<span class="badge warn">بازگشایی‌شده</span>'}</td><td>${money(v.totals?.unpaid)}</td><td><button class="btn btn-sm" onclick="showPayrollClosure('${v.id}')">مشاهده</button> <button class="btn btn-sm" onclick="downloadPayrollClosure('${v.id}')">JSON</button></td></tr>`).join('')}</tbody></table></div></div>`:''})()}`}
function editMonthlyAdjustment(pid){if(isPeriodClosed(ui.payroll.year,ui.payroll.month))return toast('ماه قطعی است؛ ابتدا آن را بازگشایی کنید',true);const a=adjFor(pid,ui.payroll.year,ui.payroll.month);openForm(`ورودی‌های دستی — ${personLabel(pid)}`,[{name:'openingBaselineDate',label:'تاریخ مبنا افتتاحیه (روز اول دوره)',type:'text'},{name:'openingBalanceToman',label:'مانده ابتدای ماه (تومان؛ خالی = محاسبه از سابقه، مثبت بدهی ما، منفی طلب ما)',type:'number'},{name:'openingNote',label:'شرح افتتاحیه؛ جایگزین مانده قبل است و هزینه جدید نیست',type:'textarea',full:true},{name:'hoursOverride',label:'ساعات کار (خالی = از سوابق شیفت)',type:'number'},{name:'estimatedHours',label:'ساعات کار تقریبی ماه (فقط برای پیش‌بینی حقوق ساعتی؛ وقتی سابقه شیفت یا اصلاح دستی نیست استفاده می‌شود)',type:'number'},{name:'overtimePerformance',label:'اضافه‌کار/کارانه',type:'number'},{name:'otherPayment',label:'سایر پرداخت',type:'number'},{name:'otherDeduction',label:'سایر کسورات',type:'number'},{name:'notes',label:'توضیحات',type:'textarea',full:true}],{...a,openingBaselineDate:a.openingBaselineDate||jDate(ui.payroll.year,ui.payroll.month,1),openingBalanceToman:a.openingBalanceToman??'',openingNote:a.openingNote||''},async o=>{if(o.estimatedHours!==''&&n(o.estimatedHours)<0)throw Error('ساعات تقریبی باید صفر یا عدد مثبت باشد');const entered=o.openingBalanceToman!==''&&o.openingBalanceToman!==null&&o.openingBalanceToman!==undefined;if(entered&&o.openingBaselineDate!==jDate(ui.payroll.year,ui.payroll.month,1))throw Error('تاریخ مبنای افتتاحیه باید روز اول دوره انتخاب‌شده باشد');if(entered&&!String(o.openingNote||'').trim())throw Error('شرح مانده افتتاحیه الزامی است');if(String(o.openingBalanceToman??'')!==String(a.openingBalanceToman??'')){const previous=calcPayrollLive(person(pid),ui.payroll.year,ui.payroll.month,{}).previousBalance;if(!confirm(`مانده قبل: ${money(previous)}؛ مانده جایگزین: ${entered?money(o.openingBalanceToman):'محاسبه از سوابق'}. این مبلغ هزینه نیست. پس از دانلود پشتیبان ثبت شود؟`))return;downloadPayrollOpeningBackup();}const before=structuredClone(a);Object.assign(a,o,{openingBaselineDate:entered?o.openingBaselineDate:'',openingBalanceMode:entered?'replacePreviousBalance':''});try{await commit('ویرایش ورودی ماهانه و مانده افتتاحیه حقوق');closeModal()}catch(e){Object.keys(a).forEach(k=>delete a[k]);Object.assign(a,before);throw e}})}
function downloadPayrollOpeningBackup(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='rayo-personnel-before-opening.json';a.click();URL.revokeObjectURL(a.href)}
function payrollSettlementRow(row,closure){const r=structuredClone(row),payments=state.payments.filter(x=>x.status!=='void'&&!x.tipSettlement&&x.personnelId===r.personnelId&&Number(x.salaryYear)===Number(closure.year)&&Number(x.salaryMonth)===Number(closure.month));r.advance=payments.filter(x=>x.type==='مساعده').reduce((v,x)=>v+n(x.amount),0);r.paid=payments.filter(x=>x.type!=='مساعده').reduce((v,x)=>v+n(x.amount),0);r.unpaid=n(row.unpaid)+n(row.advance)+n(row.paid)-r.advance-r.paid;r.status=r.unpaid<=.5?'تسویه‌شده':r.advance+r.paid>0?'پرداخت ناقص':'پرداخت‌نشده';return r}
