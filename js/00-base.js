...
function filteredPayments(){const f=ui.payments;return state.payments.filter(x=>(f.year===''||Number(x.salaryYear)===Number(f.year))&&(f.month===''||Number(x.salaryMonth)===Number(f.month))&&(!f.personnelId||x.personnelId===f.personnelId)&&(!f.type||x.type===f.type)&&(!f.method||x.method===f.method)&&(!f.from||dateCode(x.date)>=dateCode(f.from))&&(!f.to||dateCode(x.date)<=dateCode(f.to))).sort((a,b)=>dateCode(b.date)-dateCode(a.date))}
...
function effectiveSettled(g){return g.status==='تسویه‌شده'&&n(g.settledAmount)===0?n(g.totalAmount):n(g.settledAmount)}
function monthMetrics(year,month){const rows=payrollRowsForPeriod(year,month),received=state.tipGroups.filter(g=>{const q=periodOf(g.receiveDate);return q.year===year&&q.month===month&&g.status!=='ابطال‌شده'}).reduce((s,x)=>s+n(x.totalAmount),0),settled=state.tipGroups.filter(g=>{const q=periodOf(g.settlementDate);return q.year===year&&q.month===month}).reduce((s,x)=>s+effectiveSettled(x),0);return{received,settled,tipBalance:received-state.tipGroups.filter(g=>{const q=periodOf(g.receiveDate);return q.year===year&&q.month===month}).reduce((s,x)=>s+effectiveSettled(x),0),net:rows.reduce((s,x)=>s+x.netToman,0),advance:rows.reduce((s,x)=>s+x.advance,0),paid:rows.reduce((s,x)=>s+x.paid,0),unpaid:rows.reduce((s,x)=>s+x.unpaid,0)}}
...
const tipGatewayState={referencesReady:false,loading:false,submitting:false,error:null,rows:[],cache:new Map(),pendingMutation:null,pendingArchives:new Map()};
function tipQueryRange(){const f=ui.tipFilter,year=Number(f.year)||0,month=Number(f.month)||0;if(!year)return{from:'',to:''};const from=month?jDate(year,month,1):jDate(year,1,1),to=month?jDate(year,month,jMonthLength(year,month)):jDate(year,12,jMonthLength(year,12));return{from:from.replaceAll('/','-'),to:to.replaceAll('/','-')}}
...
async function tipLoadReferences(){if(tipGatewayState.referencesReady)return;const listPaths=['tipTypes','hallGroups','paymentMethods','tipStatuses','settlementMethods','months'],queries=[tipQueryAll('personnel',{includeArchived:false,sortBy:'name',descending:false}),...listPaths.map(name=>tipQueryAll(`lists.${name}`,{includeArchived:true}))],result=await Promise.all(queries);state.personnel=result[0];listPaths.forEach((name,i)=>state.lists[name]=result[i+1]);tipGatewayState.referencesReady=true}
...
async function tipRefresh({force=false,render=true}={}){const key=tipQueryKey();tipGatewayState.loading=true;tipGatewayState.error=null;if(render)renderView();try{await tipLoadReferences();let rows=!force&&tipGatewayState.cache.get(key);if(!rows){const range=tipQueryRange();rows=await tipQueryAll(TIP_QUERY_COLLECTION,{filters:tipQueryFilters(),dateField:TIP_DATE_FIELD,from:range.from,to:range.to,sortBy:TIP_DATE_FIELD,descending:true,includeArchived:false});rows=tipRowsLocal(rows);tipGatewayState.cache.set(key,rows)}tipGatewayState.rows=rows;state.tipGroups=rows;$('saveStatus').textContent=`انعام‌ها از Query دریافت شد — ${new Date().toLocaleTimeString('fa-IR')}`;return rows}catch(error){tipGatewayState.error=error;$('saveStatus').textContent='خطا در دریافت انعام — این بخش فقط‌خواندنی است';if(render)toast(error.message||'دریافت انعام انجام نشد',true);throw error}finally{tipGatewayState.loading=false;if(render)renderView()}}
...
async function tipFilterChanged(key,value){ui.tipFilter[key]=value;try{await tipRefresh()}catch(_){}}
...
viewsTips=function(){if(!tipGatewayState.referencesReady&&!tipGatewayState.loading&&!tipGatewayState.error)setTimeout(ensureTipGateway,0);const f=ui.tipFilter,rows=tipGatewayState.rows.map(g=>tipRowProjection(g)),total=rows.reduce((s,x)=>s+x.amounts.total,0),settled=rows.reduce((s,x)=>s+x.amounts.settled,0),disabled=tipGatewayState.loading||tipGatewayState.submitting||!!tipGatewayState.error?'disabled':'';return pageHead('انعام و تقسیم سهم','انعام فردی، سالن یا کلی ثبت می‌شود و سهم افراد خودکار محاسبه می‌شود.',`<button class="btn btn-primary" ${disabled} onclick="editTip()">+ ثبت انعام</button>`)+(tipGatewayState.error?`<div class="card"><div class="notice danger">${esc(tipGatewayState.error.message||'دریافت اطلاعات انعام انجام نشد. این بخش فقط‌خواندنی است.')}</div><button class="btn" onclick="tipRetryQuery()">تلاش مجدد</button></div>`:'')+`<div class="card"><div class="filter-grid"><div class="field"><label>سال</label><input type="number" value="${esc(f.year)}" onchange="tipFilterChanged('year',this.value)"></div><div class="field"><label>ماه</label><select onchange="tipFilterChanged('month',this.value)"><option value="">همه ماه‌ها</option>${state.lists.months.map((x,i)=>`<option value="${i+1}" ${String(f.month)===String(i+1)?'selected':''}>${x}</option>`).join('')}</select></div><div class="field"><label>پرسنل</label><select onchange="tipFilterChanged('personnelId',this.value)">${allPersonOptions(f.personnelId,true)}</select></div><div class="field"><label>نوع</label><select onchange="tipFilterChanged('type',this.value)"><option value="">همه</option>${state.lists.tipTypes.map(x=>`<option ${f.type===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field"><label>وضعیت</label><select onchange="tipFilterChanged('status',this.value)"><option value="">همه</option>${state.lists.tipStatuses.map(x=>`<option ${f.status===x?'selected':''}>${x}</option>`).join('')}</select></div></div></div>${tipGatewayState.loading?'<div class="card empty">در حال دریافت انعام‌ها…</div>':`<div class="grid grid-3"><div class="card kpi"><div class="label">جمع فیلترشده</div><div class="value">${money(total)}</div></div><div class="card kpi"><div class="label">تسویه‌شده</div><div class="value">${money(settled)}</div></div><div class="card kpi"><div class="label">مانده</div><div class="value">${money(total-settled)}</div></div></div><div class="card"><div class="table-wrap"><table class="data-table"><thead><tr><th>کد</th><th>تاریخ</th><th>نوع</th><th>مبلغ</th><th>افراد و سهم</th><th>تسویه</th><th>وضعیت</th><th></th></tr></thead><tbody>${rows.map(x=>{const g=x.record;return`<tr><td>${esc(g.id)}</td><td>${esc(g.receiveDate)}</td><td>${badge(g.type)}</td><td>${money(x.amounts.total)}</td><td>${x.shares.map(s=>`${esc(person(s.personnelId)?.name||s.personnelId)}: <b>${money(s.amount)}</b>`).join('<br>')}</td><td>${money(x.amounts.settled)}</td><td>${badge(g.status)}</td><td><button class="btn btn-sm" onclick="editTip('${esc(g.id)}')">ویرایش</button> <button class="btn btn-danger btn-sm" onclick="archiveTipRecord('${esc(g.id)}')">آرشیو</button></td></tr>`}).join('')||'<tr><td colspan="8" class="empty">انعامی پیدا نشد.</td></tr>'}</tbody></table></div></div>`}`}
...
const editTipQueryBase=editTip;editTip=function(id){if(tipGatewayState.loading||tipGatewayState.error)return;editTipQueryBase(id);const button=$('modalFoot')?.querySelector('.btn-primary');if(button){button.id='tipMutationSave';button.disabled=tipGatewayState.submitting}}
