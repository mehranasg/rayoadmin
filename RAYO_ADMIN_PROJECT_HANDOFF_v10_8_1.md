...
**نسخه مرجع قطعی:** Rayo Admin v10.8.1  
**تاریخ:** 2026-08-17  
**هدف:** این فایل به‌همراه ZIP سورس v10.8.1 باید برای ادامه پروژه در یک چت جدید، AI دیگر یا توسعه‌دهنده دیگر کافی باشد.  
...
v10.8.1 بر مبنای v10.8.0 ساخته شده است. معماری ایمن Seed نسخه قبل حفظ شده و مشکل دیده‌نشدن رابط «بارگذاری اطلاعات اولیه» برطرف شده است.

### تغییرات اختصاصی v10.8.1

- علت عدم مشاهده ابزار مشخص شد: کارت فقط وقتی تزریق می‌شد که URL دقیقاً `personnel.html?view=settings` بود و در بعضی مسیرهای ناوبری داخلی View تنظیمات بدون برقراری این شرط نمایش داده می‌شد.
- یک صفحه مستقل و واضح با عنوان **«مدیریت اطلاعات و بکاپ»** به منوی تنظیمات اضافه شد.
- صفحه جدید هر سه عملیات را در یک محل نشان می‌دهد: تهیه بکاپ کامل، بارگذاری اطلاعات اولیه و بازنشانی اطلاعات.
- نمایش کارت‌ها در صفحه قدیمی «تنظیمات پنل و فهرست‌ها» نیز برای سازگاری حفظ و شرط تشخیص آن مقاوم شد.
- «بارگذاری اطلاعات اولیه» و «بازنشانی اطلاعات» در دو ناحیه بصری جدا قرار گرفتند.
- Gateway قبل از هر POST یک `serverSaveToken` جدید می‌سازد و GET تأییدی باید دقیقاً همان Token را برگرداند؛ دریافت صرف یک JSON معتبر دیگر موفقیت قطعی Save محسوب نمی‌شود.
- QA قابل‌اجرا و مستقل از مسیر با نام `qa_v10_8_1.js` اضافه شد.
- نام‌های قطعی سه تب آخر انبار به این صورت تثبیت شد: «بهای تمام‌شده و بستن دوره»، «مغایرت و گردش»، «گزارش‌ها».
...
مسیر سازگار قبلی نیز حفظ شده است:
...
- 84/84 مجموع بررسی‌های Syntax، JSON، Reference، Contract و Runtime PASS
- صفحه مستقل `dataManagement`، Route منو و نمایش ابزارهای Backup/Initialize/Reset PASS
- آیکون چشم ورود ادمین، ورود پرسنل و مدیریت رمز کاربران PASS
- ماندگاری `cashreport.salesAnalytics` با `verify:true` PASS
- گردکردن میانگین فاکتور به پایین تا نزدیک‌ترین ۱۰٬۰۰۰ تومان PASS
- Save Token اجباری و تطبیق دقیق Token در GET تأییدی PASS
- Runtime test:
...
8. برای تغییر Data Schema، `RAYO_DATA_MODEL_v10_8_0.json` را هم به‌روزرسانی کند؛ v10.8.1 تغییر Schema ندارد.
...
```text
Rayo_Admin_v10_8_1_Data_Management_Visible_FLAT.zip
RAYO_ADMIN_PROJECT_HANDOFF_v10_8_1.md
RAYO_DATA_MODEL_SQL_MIGRATION_v10_8_0.md
RAYO_DATA_MODEL_v10_8_0.json
گزارش QA نسخهٔ v10.8.1 — فقط در Backup مستندات؛ نتیجهٔ جاری در docs/DOCS_CLEANUP_REPORT.md
تاریخچهٔ انتشار v10.8.1 — فقط در Git و Backup مستندات
```
