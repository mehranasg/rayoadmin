...
check('گروه مدیریت شیفت پلن در منوی نهایی ثبت شده است',/\['shift','link','[^']*','[^']*','personnel\.html\?view=shiftPlan'\]/.test(menu));
check('گروه شیفت بعد از مدیریت پرسنل درج می‌شود',menu.includes("MENU.splice(MENU.findIndex(x=>x[0]==='personnel')+1,0,['shift','link'"));
...
check('همه صفحات مشترک Cache Key جدید منو را بارگذاری می‌کنند',htmlFiles.length>=14&&htmlFiles.every(name=>fs.readFileSync(name,'utf8').includes('44-phase1-polish-v10-10-1.js?v=10.12.1-management-tabs-1')));
